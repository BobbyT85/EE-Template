...
function playSpritesheet($asset, $width, $height, $duration, $direction) {

	// NOTE - maybe add a callback function for on start, update and complete

	var fullWidth	= $width,
		fullHeight	= $height,
		widthCount 	= fullWidth / content.offsetWidth,
		heightCount = fullHeight / content.offsetHeight,
		newX, newY;
	...
	switch ($direction) {
		...
		case "h":
			...
			TweenMax.to($asset, $duration, {x:newX, ease:SteppedEase.config(widthCount), force3D:false, rotation:0.01});
			break;
		...
		case "v":
			...
			TweenMax.to($asset, $duration, {y:newY, ease:SteppedEase.config(heightCount), force3D:false, rotation:0.01});
			break;
	}
}

function resetSpritesheet($asset) { TweenMax.set($asset, {x:0, y:0}); }